...
t_vec = [0,1];                    % Start time, impact time, end time
dt    = 0.001;                      % Time step
Dt    = 0.5;                        % Extension time
y{1}  = [0.65, 0,       0; 
         Dx+0.2,  Dy-w4/2, pi/2];    % Position and orientation of the end-effector at beginning, impact, and end of (extended) phase 1
                                    % The door/plank is assumed to be at rest during this phase 
% y{2}  = [0.55,  NaN;   
%          0.32, pi/3];               % Position of the end-effector along the door and angle q4 at the beginning and end of (extended) phase 2. The begin position for q4 is not used, but is chosen such that the contact forces remain positive.
                                    % The post-impact position is determined from phase 1 for consistency
v{1}  = [0,     0,    0
         0,   0.5,    -1 ];           % (Trans. and rot.) velocity of the end-effector at beginning, impact, and end of (extended) phase 1
                                    % The door/plank is assumed to be at rest during this phase
% v{2}  = [-0.2, NaN; 0, 0];          % Velocity of the end-effector along the door and dq4/dt at the beginning and end of (extended) phase 2. The begin velocity for q4 is not used, but is chosen such that the contact forces remain positive.
                                    % The post-impact speed is determined from phase 1 using the impact map (for consistency) 

a{1}  = [0,     0,    0;
         0,   0.1,    0];% (Trans. and rot.) acceleration of the end-effector at beginning, impact, and end of (extended) phase 1
% a{1}(2,2) = 3;
% a{2}  = [0,NaN;0,0];                % Acceleration of the end-effector along the door and d2q4/dt2 at the beginning and end of (extended) phase 2. The begin acc for q4 is not used, but is chosen such that the contact forces remain positive.
...
% y_pi = [Dx*cos(q4)-Dy*sin(q4)+L1*cos(q1-q4)+L2*cos(q1+q2-q4)+L3*cos(q1+q2+q3-q4), q4];
...
% Y    = [Y;C((t_vec(2):dt:t_vec(3)).',t_vec(2),t_vec(3))*[y_pi;v_pi;a_pi;y{2}(2,:);v{2}(2,:);a{2}(2,:)]];
% Yd   = [Yd;Cd((t_vec(2):dt:t_vec(3)).',t_vec(2),t_vec(3))*[y_pi;v_pi;a_pi;y{2}(2,:);v{2}(2,:);a{2}(2,:)]];
% Ydd  = [Ydd;Cdd((t_vec(2):dt:t_vec(3)).',t_vec(2),t_vec(3))*[y_pi;v_pi;a_pi;y{2}(2,:);v{2}(2,:);a{2}(2,:)]];
% 
% q{2}  = zeros(length(t{2}),4);      qd{2} = q{2};       qdd{2} = q{2};
% for i = 1:length(t{2})
%     q4 = Y(i,2);
%     q{2}(i,:) = [InvKin([cos(q4)*Y(i,1)+w4/2*sin(q4)-Dx; sin(q4)*Y(i,1)-w4/2*cos(q4)+Dy; q4+pi/2]).',q4];
